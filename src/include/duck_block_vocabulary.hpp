...
// declared 6.5 -- so a reader trusting it concluded panduck was two spec versions behind.
// Corrected in the same change.
//
// 1.2 IS THE RENUMBER OF 6.6, NOT A DOWNGRADE. The internal 6.x line was retired; the
// vocabulary is unchanged across it, verified by diff rather than by trusting the version:
// cfd8e28 -> 1c5f2a6 moves exactly two constants, SPEC_VERSION 6.5 -> 1.2 and a new
// SPEC_VERSION_SUPERSEDES = "6.6". Nothing else is added, renamed, removed or re-valued.
//
// BEWARE THE REUSED NUMBER, per the internal-line note below: the RETIRED line had its own
// 1.1 and 1.2, and public 1.2 is not that. A check spelled `major == 1 AND minor >= 2` is
// satisfied by both and discriminates nothing. panduck is not exposed -- its only such
// function, panduck_duck_block_spec_at_least, is defined and NEVER CALLED, because a SQL
// version check REFERENCES duck_block_spec_version() and a reference binds even on an
