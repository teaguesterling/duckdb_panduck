...
void ApplyDeclarations(const std::string &decls, RunFormat &fmt) {
  ...
  while (start < decls.size()) {
    ...
    auto decl = decls.substr(start, end == std::string::npos ? std::string::npos
                                                             : end - start);
    ...
    if (colon != std::string::npos) {
      ...
      if (prop == "font-weight") {
        ...
        fmt.bold = value == "bold" || value == "bolder" ||
                   (!value.empty() && isdigit((unsigned char)value[0]) &&
                    atoi(value.c_str()) >= 600);
...
void ParseCss(const std::string &css, CssRules &rules) {
  ...
  while (pos < src.size()) {
    ...
    while (sel_start < selectors.size()) {
      ...
      auto sel = Trim(selectors.substr(sel_start, comma == std::string::npos
                                                      ? std::string::npos
                                                      : comma - sel_start));
      // `.name` or `tag.name` only -- one class, no combinators.
      auto dot = sel.find('.');
      bool simple = dot != std::string::npos &&
                    sel.find_first_of(" >+~:[") == std::string::npos &&
                    sel.find('.', dot + 1) == std::string::npos &&
                    dot + 1 < sel.size();
      if (simple) {
...
const std::set<std::string> BLOCK_LEAF = {
    "p",  "h1", "h2", "h3",  "h4",         "h5",     "h6",
...
const std::set<std::string> TRANSPARENT = {"body", "figure", "hgroup", "ul",
                                           "ol"};
...
const std::set<std::string> SKIPPED = {"head",     "script", "style",
                                       "template", "svg",    "math"};

bool IsBlockTag(const std::string &tag) {
  return BLOCK_LEAF.count(tag) || TRANSPARENT.count(tag) ||
         SECTIONING.count(tag) || SKIPPED.count(tag) || tag == "div" ||
...
RunFormat FormatFor(const pugi::xml_node &node, const std::string &tag,
                    const RunFormat &inherited, const CssRules &rules) {
...
void AppendRun(std::vector<Run> &runs, const RunFormat &fmt,
               const std::string &text, const std::string &href,
               const std::string &src) {
  if (!runs.empty() && href.empty() && src.empty() &&
      runs.back().href.empty() && runs.back().src.empty() &&
...
void CollectRuns(const pugi::xml_node &node, const RunFormat &inherited,
                 const std::string &href, const CssRules &rules,
  ...
  for (auto child : node.children()) {
    ...
    if (tag == "img") {
      ...
      runs.push_back(
          Run{inherited, child.attribute("alt").value(), std::string(), src});
      continue;
    }
    ...
    CollectRuns(child, FormatFor(child, tag, inherited, rules), child_href,
                rules, base_dir, runs);
  }
}
...
void CollectRows(const pugi::xml_node &node,
...
void EmitBlock(const pugi::xml_node &node, const std::string &element_type,
               int heading_level, const DocContext &ctx,
...
void WalkBlocks(const pugi::xml_node &node, const DocContext &ctx,
                std::vector<EpubBlock> &out, int depth) {
  for (auto child : node.children()) {
    if (child.type() == pugi::node_pcdata || child.type() == pugi::node_cdata) {
    ...
    if (tag.size() == 2 && tag[0] == 'h' && tag[1] >= '1' && tag[1] <= '6') {
      ...
      block.element_type = tag == "blockquote" ? DuckBlockTypes::TYPE_BLOCKQUOTE
                           : SECTIONING.count(tag)
                               ? DuckBlockTypes::TYPE_SECTION
      ...
      if (structural && out.size() == before + 1) {
        ...
        out.pop_back();
               ...
               std::string(child.attribute("epub:type").value())
                       .find("pagebreak") != std::string::npos) {
      ...
      if (!headers.empty() || any_row) {
        ...
        if (caption) {
          EmitBlock(caption, DuckBlockTypes::TYPE_CAPTION, 0, ctx, out,
                    depth + 1);
        }
      }
    } else if (BLOCK_LEAF.count(tag)) {
      auto type = (tag == "li" || tag == "dt" || tag == "dd")
                      ? DuckBlockTypes::TYPE_LIST_ITEM
...
pugi::xml_document ParseXml(const std::string &xml, const std::string &path,
  ...
  // flags discard it.
  auto parsed = doc.load_buffer(xml.data(), xml.size(),
                                pugi::parse_default | pugi::parse_ws_pcdata);
  if (!parsed) {
    throw InvalidInputException(
        "read_epub_blocks: %s is not well-formed XML in %s: %s", member, path,
        parsed.description());
  }
  return doc;
}
...
//! kind of difference.
void CollectMetadata(const pugi::xml_node &package,
  ...
  static const std::pair<const char *, const char *> DC_TO_PANDOC[] = {
      {"dc:creator", "author"},        {"dc:date", "date"},
  ...
  std::map<std::string, std::string>
      found; // sorted by pandoc key, which is the emission order
...
std::vector<EpubBlock> ParseEpubFile(const std::string &path) {
  ...
  std::string opf_path = container.child("container")
                             .child("rootfiles")
                             .child("rootfile")
                             .attribute("full-path")
                             .value();
  if (opf_path.empty()) {
    throw InvalidInputException(
        "read_epub_blocks: %s has a container.xml with no rootfile full-path",
        path);
  }
  ...
  if (!zip.Read(opf_path.c_str(), opf_xml)) {
    throw InvalidInputException("read_epub_blocks: %s names a package document "
  ...
  for (auto itemref : package.child("spine").children("itemref")) {
    ...
    if (type.empty() || type.find("xhtml") != std::string::npos ||
        type.find("html") != std::string::npos) {
      documents.push_back(it->second);
    }
  }

  std::map<std::string, CssRules>
      stylesheets; // member -> rules, shared across chapters
...
struct EpubRow {
  ...
  //! Defaults to `text`, which every element except `table` uses. Column 4
  //! emitted this as a hardcoded constant until spec 5.0 gave `table` a JSON
  //! content schema -- so the one element_type that needs a different encoding
  //! was the one the emitter could not express.
...
unique_ptr<FunctionData> EpubBind(ClientContext &context,
...
void RegisterEpubReaderFunction(ExtensionLoader &loader) {
  TableFunction fn("read_epub_blocks", {LogicalType::VARCHAR}, EpubScan,
                   EpubBind, EpubGlobalState::Init);
  CreateTableFunctionInfo info(std::move(fn));
  info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
  FunctionDescription desc;
  desc.parameter_names = {"file_path"};
  desc.description =
      "Read an EPUB document and return structured document blocks.";
