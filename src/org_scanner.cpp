...
//! heading.
int ParseListItem(const std::string &line, bool &ordered, int &start,
                  std::string &rest) {
  size_t i = 0;
...
std::vector<Line> ScanOrg(const std::string &src) {
  ...
  while (pos <= src.size()) {
    size_t nl = src.find('\n', pos);
    std::string raw = TrimRight(src.substr(
    ...
    if (trimmed.rfind("#+", 0) == 0) {
      ...
      if (up.rfind("BEGIN_", 0) == 0) {
        ...
        line.text =
            sp == std::string::npos ? std::string() : TrimBoth(rest.substr(sp));
        out.push_back(std::move(line));
        continue;
      }
      ...
      line.text =
          trimmed.size() > 1 ? TrimBoth(trimmed.substr(1)) : std::string();
      out.push_back(std::move(line));
      continue;
    }
