...
public:
  ...
  static void ConvertPandocAstToBlocks(const string &json,
                                       vector<Value> &result);
...
private:
  ...
  static void DuckBlocksToPandocBlocksFun(DataChunk &args,
                                          ExpressionState &state,
                                          Vector &result);

  // read_pandoc_ast(file_path VARCHAR) -> LIST(duck_block)
  // Read a Pandoc JSON file and convert to duck_blocks
  static void ReadPandocAstFun(DataChunk &args, ExpressionState &state,
                               Vector &result);
};
