...
public:
  ...
  static duckdb_yyjson::yyjson_mut_val *ConvertDbInlinesToPandocVal(
      duckdb_yyjson::yyjson_mut_doc *doc, const vector<Value> &inlines,
      idx_t start_idx, int32_t target_level, idx_t &end_idx, idx_t depth);
  ...
  static void ConvertPandocInlinesToDbInlines(const string &json,
                                              int32_t base_level,
                                              int32_t &order,
                                              vector<Value> &result,
                                              idx_t depth);
  ...
  static void
  ConvertPandocInlinesValToDbInlines(duckdb_yyjson::yyjson_val *inlines_val,
                                     int32_t base_level, int32_t &order,
...
private:
  ...
  static void PandocInlinesToDbInlinesFun(DataChunk &args,
  ...
  static void DbInlinesToPandocFun(DataChunk &args, ExpressionState &state,
                                   Vector &result);

  // pandoc_inlines_to_text(json, mode) -> VARCHAR
  // Renders Pandoc inlines to text/markdown/html
  static void PandocInlinesToTextFun(DataChunk &args, ExpressionState &state,
                                     Vector &result);
};
