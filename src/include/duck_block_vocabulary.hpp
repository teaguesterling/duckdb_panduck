...
// "1.3". The other 30 changed lines are comments -- among them IsBody's
// "NECESSARY, NOT SUFFICIENT". The body below is byte-identical to
// teaguesterling/duckdb_duck_block_utils at v3.3.0 (95a84e6); origin/main's
// header matched it when this was vendored. check-vocabulary counted
// PREDICATE_REVISION as one additive constant before the re-sync (95 local, 96
// upstream), measured rather than assumed.
//
// BODY IS A SUBTREE PROPERTY (1.4), and panduck applies it NATIVELY. A value or
// metadata row roots a subtree; it and every following row at a greater level
// is not body. duck_block_utils provides that walk as duck_blocks_body(blocks),
// but panduck's doc_section and doc_search_sections carry their own copy in
// SQL, deliberately: this header gives panduck constants, not duck_block_utils'
// functions, and a call would make both macros fail for anyone who installed
// panduck alone. The two encodings of one rule are compared by
// test/sql/doc_body_parity.test wherever both are installed (`make
// check-body-parity`).
//
// ---- history, kept because it is still true of how this file got here ----
// 1.2 -> 1.3 (#54) changed exactly two things: SPEC_VERSION 1.2 -> 1.3 and a
// new per-row IsBody(kind, element_type). That predicate calls a metadata value
// tree's inline leaves body, measured on constructs.docx, which is what led
// to 1.4 and to the native walk above.
//
// STEP 1 OF THIS FILE'S OWN RE-VENDORING GUIDANCE -- "record the provenance
// where a reader will find it" -- which panduck's copy did not carry until now.
// webbed's does. The sha is VERIFIED, not recalled: this file's body is
// byte-identical to teaguesterling/duckdb_duck_block_utils at that commit,
// diffed over all 654 lines.
//
// THE RELEASED SHA IS NAMED, not the merge commit that introduced the content.
// 1c5f2a6 (the spec 1.2 merge) and 6c1c2e5 (v3.1.0) carry an IDENTICAL header
// -- verified, since "they told me it was unchanged" is not a measurement --
// and both are true stamps. 6c1c2e5 is the more useful one: it names a RELEASE
// a reader can install, where the other names an intermediate commit they would
// have to go looking for.
//
// THE PREVIOUS CLAIM WAS WRONG, WHICH IS WORSE THAN ABSENT.
// duck_block_types.hpp said "Copied from duck_block_utils main @ b3b1e26,
// SPEC_VERSION 6.3" while the vendored copy declared 6.5 -- so a reader
// trusting it concluded panduck was two spec versions behind. Corrected in the
// same change.
//
// 1.2 IS THE RENUMBER OF 6.6, NOT A DOWNGRADE. The internal 6.x line was
// retired; the vocabulary is unchanged across it, verified by diff rather than
// by trusting the version: cfd8e28 -> 1c5f2a6 moves exactly two constants,
// SPEC_VERSION 6.5 -> 1.2 and a new SPEC_VERSION_SUPERSEDES = "6.6". Nothing
// else is added, renamed, removed or re-valued.
//
// BEWARE THE REUSED NUMBER, per the internal-line note below: the RETIRED line
// had its own 1.1 and 1.2, and public 1.2 is not that. A check spelled `major
// == 1 AND minor >= 2` is satisfied by both and discriminates nothing. panduck
// is not exposed -- its only such function, panduck_duck_block_spec_at_least,
// is defined and NEVER CALLED, because a SQL version check REFERENCES
// duck_block_spec_version() and a reference binds even on an untaken branch,
// which broke the extension-absent case. panduck's live gate is a catalog probe
// for the function it actually needs.
//
// THE INSTALLED duck_block_utils IS ALSO 1.2 AS OF 2026-09-12. duck_block_utils
// v3.1.0 is SERVED -- measured from a stock CLI into an empty
// extension_directory, not read off a changelog:
//
//     INSTALL duck_block_utils FROM community  ->  6c1c2e5,
//     duck_block_spec_version() 1.2
//
// This paragraph said "STILL 6.5 ... not yet released or served" for one day,
// which was true when written and stopped being true when the release was cut.
// Corrected rather than deleted, because the source-vs-runtime distinction it
// draws is the useful part and is permanent: vendoring is SOURCE-side, and
// nothing in panduck asserts on the number at runtime.
//
// The parsed-fixture manifest records the spec version that BUILT each fixture,
// which is a fact about the past rather than about this header. It read 6.5
// until the release landed; the drift job then reported "versions moved but
// output is byte-identical" and named duck_block_spec as the mover, which is
...
struct DuckBlockVocabulary {
  ...
  static constexpr const char *ImplicitParentOf(const char *element_type,
                                                const char *kind) {
    return SameName(kind, KIND_INLINE) ? TYPE_PLAIN
           : SameName(kind, KIND_BLOCK) &&
                   SameName(element_type, TYPE_LIST_ITEM)
               ? TYPE_LIST
           : SameName(kind, KIND_BLOCK) && SameName(element_type, TYPE_CAPTION)
               ? TYPE_FIGURE
               : "";
  }
  ...
  // by kind.
  static constexpr bool RequiresAncestor(const char *element_type,
                                         const char *kind,
                                         const char *ancestor_type) {
    return SameName(kind, KIND_INLINE) ? true
           : SameName(element_type, TYPE_LIST_ITEM)
               ? (SameName(ancestor_type, TYPE_LIST) ||
                  SameName(ancestor_type, TYPE_DEFLIST))
           : SameName(element_type, TYPE_CAPTION)
               ? (SameName(ancestor_type, TYPE_FIGURE) ||
                  SameName(ancestor_type, TYPE_TABLE))
               : false;
  }
  ...
  static constexpr bool IsBody(const char *kind, const char *element_type) {
    return (SameName(kind, KIND_BLOCK) || SameName(kind, KIND_INLINE)) &&
           !SameName(element_type, TYPE_METADATA);
  }
  ...
  static constexpr const char *ROLE_FRONTMATTER =
      "frontmatter"; // has a document body after it
  ...
  static constexpr const char *ROLE_DOCUMENT =
      "document"; // the blob IS the whole document
