...
static Value CreateDocInline(const string &inline_type, const string &content,
                             int32_t level, const map<string, string> &attrs,
                             int32_t order) {
  child_list_t<Value> struct_values;
  struct_values.push_back(
      make_pair("kind", Value(DuckBlockTypes::KIND_INLINE)));
...
static string InlineValuesToText(const vector<Value> &inlines) {
  ...
  for (auto &el : inlines) {
    ...
    string element_type =
        ch[DuckBlockTypes::ELEMENT_TYPE_IDX].IsNull()
            ? ""
            : ch[DuckBlockTypes::ELEMENT_TYPE_IDX].GetValue<string>();
    string content = ch[DuckBlockTypes::CONTENT_IDX].IsNull()
                         ? ""
                         : ch[DuckBlockTypes::CONTENT_IDX].GetValue<string>();
    if (element_type == DuckBlockTypes::INLINE_SPACE ||
        element_type == DuckBlockTypes::INLINE_SOFTBREAK) {
      out += " ";
...
static void FlattenPandocInlinesVal(yyjson_val *inlines_val, int32_t level,
  ...
  auto process_item = [&](yyjson_val *item) {
    ...
    if (strcmp(pandoc_type, "Str") == 0) {
      ...
      if (c_val && yyjson_is_arr(c_val) && yyjson_arr_size(c_val) >= 2) {
        ...
        if (code_val && yyjson_is_str(code_val)) {
          content_str =
              string(yyjson_get_str(code_val), yyjson_get_len(code_val));
        }
      }
      ...
      if (c_val && yyjson_is_arr(c_val) && yyjson_arr_size(c_val) >= 2) {
        ...
        if (m_type && yyjson_is_obj(m_type)) {
          ...
          if (mt && yyjson_is_str(mt) &&
              strcmp(yyjson_get_str(mt), "InlineMath") == 0) {
      ...
      if (c_val && yyjson_is_arr(c_val) && yyjson_arr_size(c_val) >= 3) {
        ...
        if (alt_inlines) {
          ...
          int32_t alt_order = 0;
          FlattenPandocInlinesVal(alt_inlines, level + 1, alt_order, alt_values,
                                  depth + 1);
      ...
      result.push_back(
          CreateDocInline(inline_type, alt_text, level, attrs, order++));
      return;
      ...
      if (c_val && yyjson_is_arr(c_val) && yyjson_arr_size(c_val) >= 2) {
        ...
        if (q_type && yyjson_is_obj(q_type)) {
          ...
          if (qt && yyjson_is_str(qt) &&
              strcmp(yyjson_get_str(qt), "SingleQuote") == 0) {
      ...
      result.push_back(CreateDocInline(DuckBlockTypes::INLINE_GENERIC, "",
                                       level, attrs, order++));
      if (c_val) {
        FlattenPandocInlinesVal(c_val, level + 1, order, result, depth + 1);
      }
    ...
    if (!inline_type.empty()) {
      result.push_back(
          CreateDocInline(inline_type, content_str, level, attrs, order++));
    }
  };
  ...
  if (yyjson_is_arr(inlines_val)) {
    size_t idx, max;
    yyjson_val *item;
    yyjson_arr_foreach(inlines_val, idx, max, item) { process_item(item); }
  } else if (yyjson_is_obj(inlines_val)) {
    process_item(inlines_val);
  }
}

void PandocInlineConvert::ConvertPandocInlinesValToDbInlines(
    yyjson_val *inlines_val, int32_t base_level, int32_t &order,
    vector<Value> &result, idx_t depth) {
  FlattenPandocInlinesVal(inlines_val, base_level, order, result, depth);
}

void PandocInlineConvert::ConvertPandocInlinesToDbInlines(const string &json,
                                                          int32_t base_level,
                                                          int32_t &order,
                                                          vector<Value> &result,
                                                          idx_t depth) {
...
void PandocInlineConvert::PandocInlinesToDbInlinesFun(DataChunk &args,
  ...
  for (idx_t i = 0; i < count; i++) {
    ...
    if (json_val.IsNull()) {
      result.SetValue(
          i, Value(LogicalType::LIST(DuckBlockTypes::DuckBlockType())));
      continue;
    }
...
yyjson_mut_val *PandocInlineConvert::ConvertDbInlinesToPandocVal(
    yyjson_mut_doc *doc, const vector<Value> &inlines, idx_t start_idx,
    int32_t target_level, idx_t &end_idx, idx_t depth) {
  CheckPandocDepth(depth);
  ...
  while (i < inlines.size()) {
    ...
    string content =
        children[DuckBlockTypes::CONTENT_IDX].IsNull()
            ? ""
            : children[DuckBlockTypes::CONTENT_IDX].GetValue<string>();
    int32_t level =
        children[DuckBlockTypes::LEVEL_IDX].IsNull()
            ? 1
            : children[DuckBlockTypes::LEVEL_IDX].GetValue<int32_t>();
    ...
    if (inline_type == DuckBlockTypes::INLINE_TEXT) {
      ...
      yyjson_mut_arr_add_val(arr, obj);
    } else if (inline_type == DuckBlockTypes::INLINE_BOLD ||
               inline_type == DuckBlockTypes::INLINE_ITALIC ||
               inline_type == DuckBlockTypes::INLINE_STRIKETHROUGH ||
               inline_type == DuckBlockTypes::INLINE_SUPERSCRIPT ||
               inline_type == DuckBlockTypes::INLINE_SUBSCRIPT ||
      ...
      yyjson_mut_val *nested = ConvertDbInlinesToPandocVal(
          doc, inlines, i + 1, level + 1, nested_end, depth + 1);
      ...
      if (!attrs.IsNull()) {
        ...
        for (auto &entry : map_entries) {
          ...
          auto &kv = StructValue::GetChildren(entry);
          if (kv.size() >= 2 && !kv[0].IsNull() &&
              kv[0].GetValue<string>() == "quote_type") {
      ...
      if (!source_type.empty()) {
        yyjson_mut_arr_add_strncpy(doc, classes, source_type.data(),
                                   source_type.size());
      }
      ...
      yyjson_mut_obj_add_strncpy(doc, obj, "c", placeholder.data(),
                                 placeholder.size());
      yyjson_mut_arr_add_val(arr, obj);
    }
    i++;
...
string
PandocInlineConvert::ConvertInlinesToPandocJson(const vector<Value> &inlines) {
  if (inlines.empty()) {
    return "[]";
  }
  ...
  int32_t target_level =
      first_children[DuckBlockTypes::LEVEL_IDX].IsNull()
          ? 1
  ...
  yyjson_mut_val *arr =
      ConvertDbInlinesToPandocVal(doc, inlines, 0, target_level, end_idx, 1);
...
static string RenderInlinesToTextVal(yyjson_val *inlines_val,
                                     const string &mode, idx_t depth) {
  CheckPandocDepth(depth);
  if (!inlines_val) {
    return "";
  }
...
void PandocInlineConvert::DbInlinesToPandocFun(DataChunk &args,
                                               ExpressionState &state,
                                               Vector &result) {
...
void PandocInlineConvert::PandocInlinesToTextFun(DataChunk &args,
                                                 ExpressionState &state,
                                                 Vector &result) {
  auto &json_vec = args.data[0];
  auto count = args.size();
...
static void DbInlinesToPandocNestedFun(DataChunk &args, ExpressionState &state,
                                       Vector &result) {
  auto &list_vec = args.data[0];
  auto count = args.size();
