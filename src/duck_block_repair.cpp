...
string Field(const Value &v, idx_t idx) {
  ...
  return (idx < c.size() && !c[idx].IsNull()) ? c[idx].GetValue<string>()
                                              : string();
}

int32_t IntField(const Value &v, idx_t idx, int32_t def) {
  auto &c = StructValue::GetChildren(v);
  return (idx < c.size() && !c[idx].IsNull()) ? c[idx].GetValue<int32_t>()
                                              : def;
}
...
bool Satisfied(const El &e, const vector<Frame> &stack) {
  const char *parent =
      DuckBlockTypes::ImplicitParentOf(e.type.c_str(), e.kind.c_str());
  if (parent[0] == '\0') {
    return true;
  }
  for (auto &f : stack) {
    ...
    if (f.kind == DuckBlockTypes::KIND_BLOCK &&
        DuckBlockTypes::RequiresAncestor(e.type.c_str(), e.kind.c_str(),
                                         f.type.c_str())) {
      return true;
    }
  }
  ...
  while (i < els.size()) {
    ...
    while (j < els.size()) {
      ...
      const bool same_orphan =
          x.level == run_level &&
          (run_is_inline ? x.kind == DuckBlockTypes::KIND_INLINE
                         : (x.kind == run_kind && x.type == run_type));
    ...
    const int32_t shift =
        run_level <= 1 ? 1 : 0; // the wrapper cannot sit at level 0
    El w;
