...
const Mapping MAPPINGS[] = {
    ...
    {"Header", "block", "heading", STATUS_MAPPED,
     "heading_level 1-6 in attributes; id from Attr"},
    {"Para", "block", "paragraph", STATUS_MAPPED,
     "text-only runs flatten to content; rich runs emit inline children"},
    {"Plain", "block", "paragraph", STATUS_MAPPED,
     "same as Para, with plain=true in attributes"},
    {"CodeBlock", "block", "code", STATUS_MAPPED,
     "language taken from the first Attr class"},
    {"BlockQuote", "block", "blockquote", STATUS_MAPPED,
     "nested blocks flattened, depth carried in level"},
    {"BulletList", "block", "list", STATUS_MAPPED, "list_type=bullet"},
    {"OrderedList", "block", "list", STATUS_MAPPED,
     "list_type=ordered, plus start/style/delimiter"},
    {"Table", "block", "table", STATUS_MAPPED,
     "full structure preserved as json encoding"},
    {"HorizontalRule", "block", "hr", STATUS_MAPPED, ""},
    {"RawBlock", "block", "raw", STATUS_MAPPED,
     "source format recorded in attributes"},
    ...
    {"Figure", "block", "pandoc:figure", STATUS_PLANNED,
     "pandoc 3.0+; spec'd but no code path; currently dropped"},
    {"Null", "block", nullptr, STATUS_DROPPED,
     "empty block, intentionally yields no element"},
    ...
    {"Math", "inline", "math", STATUS_MAPPED,
     "InlineMath vs DisplayMath recorded in attributes"},
    {"Link", "inline", "link", STATUS_MAPPED, "href/title from Target"},
    {"Image", "inline", "image", STATUS_MAPPED, "src/title from Target"},
    {"RawInline", "inline", "raw", STATUS_MAPPED,
     "source format recorded in attributes"},
...
unique_ptr<FunctionData> PandocAstMapBind(ClientContext &,
...
void PandocAstMapScan(ClientContext &, TableFunctionInput &input,
                      DataChunk &output) {
  auto &state = input.global_state->Cast<PandocAstMapGlobalState>();

  idx_t count = 0;
  while (state.offset < pandoc_ast::MAPPING_COUNT &&
    ...
    output.SetValue(2, count,
                    m.element_type ? Value(m.element_type)
...
void RegisterPandocAstMapFunction(ExtensionLoader &loader) {
  TableFunction fn("panduck_pandoc_ast_map", {}, PandocAstMapScan,
                   PandocAstMapBind, PandocAstMapGlobalState::Init);
