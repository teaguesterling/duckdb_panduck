...
private:
  ...
  void Push(const char *type, const std::string &content, int level,
  ...
  void Cells(yyjson_val *cells) {
    ...
    yyjson_arr_foreach(cells, idx, max, cell) {
      ...
      auto source =
          TrimTrailingNewline(JoinSource(yyjson_obj_get(cell, "source")));
      ...
      // flattened away.
      Push(DuckBlockTypes::TYPE_DIV, std::string(), 1, {}, {},
  ...
  void Outputs(yyjson_val *outputs) {
    ...
    yyjson_arr_foreach(outputs, idx, max, out) {
      ...
      Push(DuckBlockTypes::TYPE_DIV, std::string(), 2, {}, {},
           kind.empty() ? "output" : kind);
  ...
  void Metadata(yyjson_val *nb_meta) {
    ...
    if (authors && yyjson_is_arr(authors)) {
      ...
      yyjson_arr_foreach(authors, idx, max, a) {
        auto name = yyjson_is_str(a) ? std::string(yyjson_get_str(a))
                                     : StrField(a, "name");
        if (!name.empty()) {
...
unique_ptr<FunctionData> IpynbFileBind(ClientContext &context,
...
unique_ptr<FunctionData> IpynbStringBind(ClientContext &context,
                                         TableFunctionBindInput &input,
                                         vector<LogicalType> &return_types,
                                         panduck::BindNames &names) {
  readers::RequireReaderEnabled(context, "ipynb");
...
void RegisterIpynbReader(ExtensionLoader &loader) {
  {
    TableFunction file_fn("read_ipynb_blocks", {LogicalType::VARCHAR},
                          IpynbScan, IpynbFileBind, IpynbGlobalState::Init);
  ...
  {
    TableFunction string_fn("read_ipynb_blocks_string", {LogicalType::VARCHAR},
                            IpynbScan, IpynbStringBind, IpynbGlobalState::Init);
    ...
    desc.description = "Parse a Jupyter Notebook JSON string and return "
                       "structured document blocks.";
    desc.examples = {
        "SELECT * FROM read_ipynb_blocks_string('{\"cells\": [], \"metadata\": "
