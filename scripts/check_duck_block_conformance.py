...
def blk(element_type, level, element_order, content="NULL", kind="block"):
    ...
    return (
        "{'kind':'%s','element_type':'%s','content':%s,'level':%d::INTEGER,"
...
SELF_TESTS = [
    (
        ...
        doc(blk("paragraph", 1, 0, "hello")),
        True,
        [],
    ),
    (
        "duplicate element_order",
        doc(blk("paragraph", 1, 0, "a"), blk("paragraph", 1, 0, "b")),
        False,
        [],
    ),
    (
        "level jumps by more than one",
        doc(blk("paragraph", 1, 0), blk("paragraph", 3, 1, "orphan")),
        False,
        [],
    ),
    ("level below 1", doc(blk("metadata", 0, 0, "title")), False, []),
    (
        "element_type outside the vocabulary",
        doc(blk("zzzz_not_a_type", 1, 0, "x")),
        True,
        ["zzzz_not_a_type"],
    ),
]
...
def self_test(macros):
    ...
    for i, (_name, document, _v, _u) in enumerate(SELF_TESTS):
        sql.append(
            "SELECT 'st%d' || '%s' || duck_blocks_are_valid(%s)::VARCHAR || '%s' || "
            "coalesce(list_aggregate(duck_blocks_undeclared_types(%s), "
            "'string_agg', ','), '');" % (i, SEP, document, SEP, document)
        )
    got = {}
    for line in run_sql("\n".join(sql)):
