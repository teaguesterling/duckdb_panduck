...
void RegisterPandocReader(ExtensionLoader &loader) {
	{
		...
		desc.description = "Read a Pandoc AST JSON file and return structured document blocks.";
	...
	// tests.
	{
		...
		desc.description = "Parse a Pandoc AST JSON string and return structured document blocks.";
		desc.examples = {"SELECT * FROM read_pandoc_blocks_string('{\"pandoc-api-version\": "
		                 "[1,23,1], \"meta\": {}, \"blocks\": []}')"};
		desc.categories = {"panduck"};
		info.descriptions.push_back(desc);
		loader.RegisterFunction(std::move(info));
	...
	{
		CreateScalarFunctionInfo info(ScalarFunction("panduck_pandoc_ast_to_blocks", {LogicalType::VARCHAR},
		                                             DuckBlockTypes::DuckBlockListType(), PandocAstToBlocksFun));
		info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
		FunctionDescription desc;
		desc.parameter_names = {"pandoc_ast_json"};
		desc.description = "Convert a Pandoc AST JSON string into a list of duck_blocks.";
