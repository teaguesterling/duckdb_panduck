...
void ParseInlines(const std::string &s, int level,
                  std::vector<RstInline> &out) {
  std::string plain;
  size_t i = 0;
  auto flush = [&]() {
  ...
  auto emit = [&](const char *type, const std::string &content,
  ...
  while (i < s.size()) {
    if (s.compare(i, 2, "``") == 0) {
      ...
      if (close != std::string::npos) {
        emit(DuckBlockTypes::INLINE_CODE, s.substr(i + 2, close - i - 2),
             std::string());
        i = close + 2;
        continue;
      }
    ...
    if (s.compare(i, 2, "**") == 0) {
      ...
      if (close != std::string::npos && close > i + 2) {
        emit(DuckBlockTypes::INLINE_BOLD, s.substr(i + 2, close - i - 2),
             std::string());
        i = close + 2;
        continue;
      }
    ...
    if (s[i] == '*') {
      ...
      if (close != std::string::npos && close > i + 1) {
        emit(DuckBlockTypes::INLINE_ITALIC, s.substr(i + 1, close - i - 1),
             std::string());
        i = close + 1;
        continue;
      }
...
std::vector<std::string> SimpleCells(const std::string &row,
...
private:
  ...
  void Emit(const char *type, const std::string &text, int level,
    ...
    if (runs.size() == 1 &&
        runs[0].element_type == DuckBlockTypes::INLINE_TEXT) {
  ...
  std::string RawBody(size_t from, size_t to, int base) const {
    ...
    for (size_t j = from; j < to; j++) {
      std::string t =
          lines_[j].kind == LineKind::BLANK ? std::string() : lines_[j].text;
      if (!body.empty()) {
        body += "\n";
      }
      body += t;
  ...
  void Run(size_t from, size_t to, int depth, int base) {
    ...
    auto flush = [&]() {
      ...
      if (text.size() >= 2 && text.compare(text.size() - 2, 2, "::") == 0) {
        ...
        text =
            text.size() == 2 ? std::string() : text.substr(0, text.size() - 1);
      }
      if (!text.empty()) {
    ...
    for (size_t i = from; i < to; i++) {
      ...
      case LineKind::COMMENT: {
        ...
        if (i + 1 < to && lines_[i + 1].kind != LineKind::BLANK &&
            lines_[i + 1].indent > line.indent) {
      ...
      case LineKind::FIELD: {
        ...
        while (j < to && lines_[j].kind == LineKind::FIELD) {
          ...
          Emit(DuckBlockTypes::TYPE_LIST_ITEM, lines_[j].name, depth + 1,
               DuckBlockTypes::ROLE_TERM);
          j++;
          ...
          // list in two.
          while (j < to && lines_[j].kind == LineKind::TEXT &&
                 lines_[j].indent > field_indent) {
            value += (value.empty() ? "" : " ") + lines_[j].text;
            j++;
          }
          Emit(DuckBlockTypes::TYPE_LIST_ITEM, value, depth + 1,
      ...
      case LineKind::TEXT: {
        ...
        if (para.empty() && i + 1 < to && lines_[i + 1].indent > line.indent &&
          ...
          Emit(DuckBlockTypes::TYPE_LIST_ITEM, def, depth + 1,
               DuckBlockTypes::ROLE_DEFINITION);
          i = end - 1;
          continue;
        }
  ...
  size_t List(size_t i, size_t to, int depth) {
    ...
    RstBlock list;
    list.element_type = DuckBlockTypes::TYPE_LIST;
    list.list_type = ordered ? DuckBlockTypes::LIST_TYPE_ORDERED
    ...
    while (j < to) {
      ...
      if (line.kind == LineKind::BLANK) {
        if (j + 1 < to &&
            (lines_[j + 1].kind == LineKind::BULLET ||
             lines_[j + 1].kind == LineKind::ENUM) &&
      ...
      if ((line.kind != LineKind::BULLET && line.kind != LineKind::ENUM) ||
          line.indent != indent || line_ordered != ordered) {
...
unique_ptr<FunctionData> RstFileBind(ClientContext &context,
...
unique_ptr<FunctionData> RstStringBind(ClientContext &context,
                                       TableFunctionBindInput &input,
                                       vector<LogicalType> &return_types,
                                       panduck::BindNames &names) {
  readers::RequireReaderEnabled(context, "rst");
...
void RegisterRstReader(ExtensionLoader &loader) {
  {
    TableFunction file_fn("read_rst_blocks", {LogicalType::VARCHAR}, RstScan,
                          RstFileBind, RstGlobalState::Init);
  ...
  {
    TableFunction string_fn("read_rst_blocks_string", {LogicalType::VARCHAR},
                            RstScan, RstStringBind, RstGlobalState::Init);
    ...
    desc.description = "Parse a reStructuredText (RST) string and return "
                       "structured document blocks.";
    desc.examples = {
        "SELECT * FROM read_rst_blocks_string('Title\\n=====\\n\\nParagraph')"};
    desc.categories = {"panduck"};
    info.descriptions.push_back(desc);
    loader.RegisterFunction(std::move(info));
  }
