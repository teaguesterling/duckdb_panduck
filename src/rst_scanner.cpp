  ...
  while (pos <= src.size()) {
    ...
    if ((t[0] == '-' || t[0] == '*' || t[0] == '+') && t.size() > 1 &&
      ...
      line.text = TrimBoth(t.substr(2));
      {
        ...
        line.text_col =
            line.indent + static_cast<int>(k == std::string::npos ? 2 : k);
      }
      out.push_back(std::move(line));
      continue;
    }
    {
      ...
      if (d > 0 && d < t.size() && (t[d] == '.' || t[d] == ')') &&
        ...
        line.text = TrimBoth(t.substr(d + 2));
        {
          ...
          line.text_col = line.indent +
                          static_cast<int>(k == std::string::npos ? d + 2 : k);
        }
        out.push_back(std::move(line));
        continue;
      }
