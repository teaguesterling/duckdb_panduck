...
void PushText(std::vector<TxInline> &out, const std::string &text, int level) {
  ...
  if (!out.empty() && out.back().element_type == DuckBlockTypes::INLINE_TEXT &&
      out.back().attributes.empty()) {
...
void PushWrapped(std::vector<TxInline> &out, const char *type,
                 const std::string &inner, int level) {
  TxInline node;
  ...
  if (children.size() == 1 &&
      children[0].element_type == DuckBlockTypes::INLINE_TEXT &&
...
bool TryDelim(const std::string &s, size_t &i, const char *open,
              const char *type, int level, std::vector<TxInline> &out,
              std::string &pending) {
...
void ParseInlines(const std::string &s, int level, std::vector<TxInline> &out) {
  ...
  for (size_t i = 0; i < s.size(); i++) {
    ...
    if (TryDelim(s, i, "**", DuckBlockTypes::INLINE_BOLD, level, out,
                 pending) ||
        TryDelim(s, i, "__", DuckBlockTypes::INLINE_ITALIC, level, out,
                 pending) ||
        TryDelim(s, i, "??", DuckBlockTypes::INLINE_CITE, level, out,
                 pending) ||
        TryDelim(s, i, "*", DuckBlockTypes::INLINE_BOLD, level, out, pending) ||
        TryDelim(s, i, "_", DuckBlockTypes::INLINE_ITALIC, level, out,
                 pending) ||
        TryDelim(s, i, "@", DuckBlockTypes::INLINE_CODE, level, out, pending) ||
        TryDelim(s, i, "-", DuckBlockTypes::INLINE_STRIKETHROUGH, level, out,
                 pending) ||
        TryDelim(s, i, "+", DuckBlockTypes::INLINE_UNDERLINE, level, out,
                 pending) ||
        TryDelim(s, i, "^", DuckBlockTypes::INLINE_SUPERSCRIPT, level, out,
                 pending) ||
        TryDelim(s, i, "~", DuckBlockTypes::INLINE_SUBSCRIPT, level, out,
                 pending) ||
...
void AttachInlines(TxBlock &block, const std::string &text) {
  ...
  if (inl.size() == 1 && inl[0].element_type == DuckBlockTypes::INLINE_TEXT &&
      inl[0].attributes.empty()) {
  ...
  std::vector<TxBlock> Build(const std::string &src) {
    ...
    for (size_t i = 0; i < lines.size(); i++) {
      ...
      case LineKind::HEADING: {
        ...
        b.level = 1;
        b.attributes[DuckBlockTypes::ATTR_HEADING_LEVEL] =
            std::to_string(ln.level);
  ...
  void ListItem(const Line &ln) {
    ...
    for (size_t k = common; k < m.size(); k++) {
      ...
      b.attributes[DuckBlockTypes::ATTR_ORDERED_LEGACY] =
          lt == std::string(DuckBlockTypes::LIST_TYPE_ORDERED) ? "true"
                                                               : "false";
    ...
    if (ln.definition) {
      ...
      def.attributes[DuckBlockTypes::ATTR_ROLE] =
...
unique_ptr<FunctionData> TxFileBind(ClientContext &context,
...
unique_ptr<FunctionData> TxStringBind(ClientContext &context,
                                      TableFunctionBindInput &input,
                                      vector<LogicalType> &return_types,
                                      panduck::BindNames &names) {
  readers::RequireReaderEnabled(context, "textile");
...
void RegisterTextileReader(ExtensionLoader &loader) {
  {
    TableFunction file_fn("read_textile_blocks", {LogicalType::VARCHAR}, TxScan,
                          TxFileBind, TxGlobalState::Init);
    CreateTableFunctionInfo info(std::move(file_fn));
    info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
    FunctionDescription desc;
    desc.parameter_names = {"file_path"};
    desc.description =
        "Read a Textile document and return structured document blocks.";
  ...
  {
    TableFunction string_fn("read_textile_blocks_string",
                            {LogicalType::VARCHAR}, TxScan, TxStringBind,
                            TxGlobalState::Init);
    ...
    desc.description =
        "Parse a Textile string and return structured document blocks.";
    desc.examples = {
        "SELECT * FROM read_textile_blocks_string('h1. Header\\n\\n*bold*')"};
    desc.categories = {"panduck"};
    info.descriptions.push_back(desc);
    loader.RegisterFunction(std::move(info));
  }
