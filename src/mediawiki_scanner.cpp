...
const char *const kHtmlBlocks[] = {
    "blockquote", "syntaxhighlight", "source", "pre",  "references",
...
bool IsBehaviorSwitch(const std::string &t) {
  if (t.size() < 5 || !StartsWith(t, "__") ||
      t.compare(t.size() - 2, 2, "__") != 0) {
    return false;
  }
...
std::vector<Line> ScanMediaWiki(const std::string &src) {
  ...
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    if (t[0] == '<' && t.size() > 2) {
      ...
      while (
          np < t.size() &&
          (std::isalnum(static_cast<unsigned char>(t[np])) || t[np] == '_')) {
        np++;
      }
      std::string tag;
      for (size_t k = 1; k < np; k++) {
        tag +=
            static_cast<char>(std::tolower(static_cast<unsigned char>(t[k])));
      }
      bool known = false;
    ...
    if (t[0] == '<' && t.back() == '>') {
      ...
      if (outside.empty()) {
        ...
        bool inline_owned =
            tag == "ref" || tag == "nowiki" || tag == "code" || tag == "math";
        if (!inline_owned) {
