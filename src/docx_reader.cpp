...
int ParagraphHeadingLevel(const pugi::xml_node &ppr,
...
void CollectDocxMetadata(const std::string &core_xml,
                         std::vector<DocxBlock> &out) {
  if (core_xml.empty()) {
    return;
  }
  pugi::xml_document doc;
...
std::vector<DocxBlock> ParseDocxFile(const std::string &path) {
  ...
  auto parsed = doc.load_buffer(document_xml.data(), document_xml.size(),
                                pugi::parse_default | pugi::parse_ws_pcdata);
  if (!parsed) {
    throw InvalidInputException(
        "read_docx_blocks: word/document.xml is not well-formed XML in %s: %s",
  ...
  if (!numbering_xml.empty() &&
      num_doc.load_buffer(numbering_xml.data(), numbering_xml.size(),
                          pugi::parse_default)) {
  ...
  if (!rels_xml.empty() &&
      rels_doc.load_buffer(rels_xml.data(), rels_xml.size())) {
  ...
  if (!footnotes_xml.empty() &&
      fn_doc.load_buffer(footnotes_xml.data(), footnotes_xml.size(),
                         pugi::parse_default | pugi::parse_ws_pcdata)) {
  ...
  std::function<void(pugi::xml_node)> flatten_blocks =
  ...
  for (auto node : body_nodes) {
    ...
    if (node_name == "w:tbl") {
      ...
      for (auto tr : node.children("w:tr")) {
        ...
        // does this.
        bool is_header = tr.child("w:trPr").child("w:tblHeader") ||
                         tr.select_node(".//w:tblHeader").node();
    ...
    bool quote_style = pstyle_name == "BlockText" || pstyle_name == "Quote" ||
                       pstyle_name == "IntenseQuote" ||
                       pstyle_name == "BlockQuote";
    int ind_left = ppr.child("w:ind").attribute("w:left").as_int(0);
    bool is_quote =
        level == 0 && list_depth == 0 && (quote_style || ind_left >= 720);
    ...
    // together.
    bool is_code_para =
        pstyle_name == "SourceCode" || pstyle_name == "PreformattedText";
    ...
    std::function<void(pugi::xml_node, const std::string &)> walk_inline =
        [&](pugi::xml_node parent, const std::string &href) {
          for (auto child : parent.children()) {
            ...
            } else if (ctag == "w:ins" || ctag == "w:smartTag" ||
                       ctag == "w:fldSimple" || ctag == "w:sdt" ||
                       ctag == "w:sdtContent" || ctag == "w:bdo" ||
                       ctag == "w:dir") {
    ...
    if (list_depth > 0 &&
        open_ordered.size() == static_cast<size_t>(list_depth) &&
    ...
    while (open_ordered.size() < static_cast<size_t>(list_depth)) {
      ...
      l.list_type = list_ordered ? DuckBlockTypes::LIST_TYPE_ORDERED
    ...
    // the metadata below.
    if ((pstyle_name == "Title" || pstyle_name == "Author" ||
         pstyle_name == "Subtitle" || pstyle_name == "Date") &&
    ...
    if (!any_format || level > 0) {
      ...
      for (auto &r : runs) {
        block.inlines.push_back(DocxInline{
...
unique_ptr<FunctionData> DocxBind(ClientContext &context,
...
void RegisterDocxReaderFunction(ExtensionLoader &loader) {
  TableFunction fn("read_docx_blocks", {LogicalType::VARCHAR}, DocxScan,
                   DocxBind, DocxGlobalState::Init);
  CreateTableFunctionInfo info(std::move(fn));
  info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
  FunctionDescription desc;
  desc.parameter_names = {"file_path"};
  desc.description =
      "Read a DOCX document and return structured document blocks.";
