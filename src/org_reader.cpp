...
const InlineMarker MARKERS[] = {
    {'*', DuckBlockTypes::INLINE_BOLD},
    {'/', DuckBlockTypes::INLINE_ITALIC},
    {'_', DuckBlockTypes::INLINE_UNDERLINE},
    {'=', DuckBlockTypes::INLINE_CODE},
    {'~', DuckBlockTypes::INLINE_CODE},
    {'+', DuckBlockTypes::INLINE_STRIKETHROUGH},
};
...
bool OpensHere(const std::string &s, size_t i) {
  if (i > 0 && !isspace(static_cast<unsigned char>(s[i - 1])) &&
      s[i - 1] != '(' && s[i - 1] != '[') {
    return false;
  }
...
private:
  ...
  int Depth() const { return lists_.empty() ? 1 : lists_.back().second + 1; }
  ...
  void FlushParagraph() {
    ...
    if (item.element_type == DuckBlockTypes::TYPE_LIST_ITEM &&
        item.inlines.empty() && item.content.empty()) {
      item.content = text;
      return;
    }
    Emit(DuckBlockTypes::TYPE_PARAGRAPH, text, Depth());
  ...
  void Item(const Line &line) {
    ...
    if (line.definition) {
      ...
      // the meaning.
      Emit(DuckBlockTypes::TYPE_LIST_ITEM, line.term, item_level,
  ...
  void Keyword(const Line &line) {
    if (line.key != "TITLE" && line.key != "AUTHOR" && line.key != "DATE") {
      ...
      // #+notarealkeyword.
      b.content =
          line.raw.empty() ? "#+" + line.key + ": " + line.text : line.raw;
    ...
    std::string key = line.key == "TITLE"    ? "title"
                      : line.key == "AUTHOR" ? "author"
                                             : "date";
...
unique_ptr<FunctionData> OrgFileBind(ClientContext &context,
...
unique_ptr<FunctionData> OrgStringBind(ClientContext &context,
                                       TableFunctionBindInput &input,
                                       vector<LogicalType> &return_types,
                                       panduck::BindNames &names) {
  readers::RequireReaderEnabled(context, "org");
...
void RegisterOrgReader(ExtensionLoader &loader) {
  {
    TableFunction file_fn("read_org_blocks", {LogicalType::VARCHAR}, OrgScan,
                          OrgFileBind, OrgGlobalState::Init);
  ...
  {
    TableFunction string_fn("read_org_blocks_string", {LogicalType::VARCHAR},
                            OrgScan, OrgStringBind, OrgGlobalState::Init);
    CreateTableFunctionInfo info(std::move(string_fn));
    info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
    FunctionDescription desc;
    desc.parameter_names = {"org_text"};
    desc.description =
        "Parse an Emacs Org-mode string and return structured document blocks.";
    desc.examples = {
        "SELECT * FROM read_org_blocks_string('* Heading\\nParagraph')"};
    desc.categories = {"panduck"};
    info.descriptions.push_back(desc);
    loader.RegisterFunction(std::move(info));
  }
