...
struct IpynbBlock {
  ...
  std::string content;
  std::string
      encoding; //!< left at its default; a raw block's format is NOT here
  std::string
      raw_format; //!< `raw` only: attributes['format'] -- the embedded format
  std::string language; //!< `code` only
  std::string
      source_type; //!< div: the cell or output kind; value: the original field
  int level = 1;
