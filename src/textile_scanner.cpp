...
//! then untouched.
bool ParseMarker(const std::string &t, std::string &name, std::string &style,
                 std::string &css_class, std::string &id, size_t &body_start) {
  size_t i = 0;
  ...
  for (size_t li = 0; li < lines.size(); li++) {
    ...
    if (t.front() == '<' && t.size() > 2) {
      ...
      static const char *const kBlocks[] = {
          "dl", "table", "div", "ul", "ol", "blockquote", "pre", "p", nullptr};
      bool known = false;
