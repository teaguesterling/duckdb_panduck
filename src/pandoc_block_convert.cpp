...
static Value CreateDocValue(const string &value_type, const string &content,
                            const map<string, string> &attrs, int32_t order,
                            const Value &level) {
  child_list_t<Value> struct_values;
    ...
    yyjson_arr_foreach(kvs_val, idx, max, kv) {
      if (yyjson_is_arr(kv) && yyjson_arr_size(kv) >= 2) {
        ...
        if (k && yyjson_is_str(k) && v && yyjson_is_str(v)) {
          attr.key_values.emplace_back(
              string(yyjson_get_str(k), yyjson_get_len(k)),
...
static bool IsReservedAttrKey(const string &key) {
  return key == "id" || key == "class" ||
         key == DuckBlockTypes::ATTR_HEADING_LEVEL || key == "language" ||
         key == DuckBlockTypes::ATTR_LIST_TYPE || key == "format" ||
         key == "src" || key == "alt" || key == "title" || key == "href" ||
         key == "quote_type" || key == "display";
}

static void StorePandocAttr(const PandocAttr &attr,
...
static string ExtractInlinesTextVal(yyjson_val *inlines_arr, idx_t depth = 0) {
  ...
  auto process_item = [&](yyjson_val *item) {
    ...
    if (strcmp(t, "Str") == 0) {
      ...
      result += "\n";
    } else if (strcmp(t, "Code") == 0 || strcmp(t, "Math") == 0 ||
               strcmp(t, "RawInline") == 0) {
...
static void AppendRowCellsJson(yyjson_val *row, string &out) {
  out += "[";
  yyjson_val *cells = (row && yyjson_is_arr(row) && yyjson_arr_size(row) >= 2)
                          ? yyjson_arr_get(row, 1)
                          : nullptr;
  bool first = true;
  if (cells && yyjson_is_arr(cells)) {
    ...
    yyjson_arr_foreach(cells, idx, max, cell) {
      ...
      yyjson_val *cell_blocks =
          (cell && yyjson_is_arr(cell) && yyjson_arr_size(cell) >= 5)
              ? yyjson_arr_get(cell, 4)
              : nullptr;
...
static string ProjectTableToNativeJson(yyjson_val *c_val) {
  ...
  yyjson_val *head_rows =
      (head && yyjson_is_arr(head) && yyjson_arr_size(head) >= 2)
          ? yyjson_arr_get(head, 1)
          : nullptr;
...
static bool InlinesAreTextOnly(const vector<Value> &inlines) {
  for (auto &el : inlines) {
    ...
    auto element_type =
        children[DuckBlockTypes::ELEMENT_TYPE_IDX].GetValue<string>();
    ...
    if (element_type != DuckBlockTypes::INLINE_TEXT &&
        element_type != DuckBlockTypes::INLINE_SPACE) {
      return false;
    }
  }
  return true;
...
static void ProcessPandocBlockVal(yyjson_val *block_val, int32_t &order,
  ...
  const int32_t effective_level =
      (parent_div_level == 0) ? 1 : parent_div_level + 1;
  ...
  if (strcmp(pandoc_type, "Header") == 0) {
  ...
  } else if (strcmp(pandoc_type, "Para") == 0 ||
             strcmp(pandoc_type, "Plain") == 0) {
    ...
    block_type = (strcmp(pandoc_type, "Plain") == 0)
                     ? DuckBlockTypes::TYPE_PLAIN
    ...
    result.push_back(
        CreateDocBlock(block_type, "", attrs, order++, encoding, block_level));
    if (c_val && yyjson_is_arr(c_val)) {
      size_t idx, max;
      yyjson_val *child_block;
      yyjson_arr_foreach(c_val, idx, max, child_block) {
        ProcessPandocBlockVal(child_block, order, result, depth + 1,
                              effective_level);
      }
    }
    return;
  } else if (strcmp(pandoc_type, "BulletList") == 0 ||
             strcmp(pandoc_type, "OrderedList") == 0) {
    ...
    attrs[DuckBlockTypes::ATTR_LIST_TYPE] =
        is_ordered ? DuckBlockTypes::LIST_TYPE_ORDERED
    ...
    if (is_ordered && c_val && yyjson_is_arr(c_val) &&
        yyjson_arr_size(c_val) >= 2) {
      ...
      if (list_attrs && yyjson_is_arr(list_attrs) &&
          yyjson_arr_size(list_attrs) >= 3) {
        ...
        for (idx_t k = 1; k < 3; k++) {
          ...
          if (tag && yyjson_is_str(tag)) {
            attrs[k == 1 ? "number_style" : "number_delim"] =
                yyjson_get_str(tag);
          }
        }
      }
    ...
    if (items_arr && yyjson_is_arr(items_arr)) {
      ...
      yyjson_arr_foreach(items_arr, idx, max, item_val) {
        ...
        result.push_back(CreateDocBlock(DuckBlockTypes::TYPE_LIST_ITEM, "",
                                        item_attrs, order++, "text",
        ...
        if (item_val && yyjson_is_arr(item_val)) {
          ...
          yyjson_arr_foreach(item_val, bidx, bmax, item_block) {
            ProcessPandocBlockVal(item_block, order, result, depth + 1,
                                  effective_level + 1);
          }
        }
      }
    ...
    if (c_val && yyjson_is_arr(c_val)) {
      ...
      yyjson_arr_foreach(c_val, idx, max, pair) {
        ...
        result.push_back(CreateDocBlock(DuckBlockTypes::TYPE_LIST_ITEM, "",
                                        term_attrs, order++, "text",
                                        Value(effective_level + 1)));
        // The term is a bare inline run, so its block-level home is `plain`.
        map<string, string> no_attrs;
        result.push_back(CreateDocBlock(
            DuckBlockTypes::TYPE_PLAIN, ExtractInlinesTextVal(term_inlines),
        ...
        if (definitions && yyjson_is_arr(definitions)) {
          ...
          yyjson_arr_foreach(definitions, didx, dmax, def_blocks) {
            ...
            def_attrs[DuckBlockTypes::ATTR_ROLE] =
                DuckBlockTypes::LIST_TYPE_DEFINITION;
            result.push_back(CreateDocBlock(DuckBlockTypes::TYPE_LIST_ITEM, "",
                                            def_attrs, order++, "text",
                                            Value(effective_level + 1)));
            if (def_blocks && yyjson_is_arr(def_blocks)) {
              size_t bidx, bmax;
              yyjson_val *blk;
              yyjson_arr_foreach(def_blocks, bidx, bmax, blk) {
                ProcessPandocBlockVal(blk, order, result, depth + 1,
                                      effective_level + 1);
              }
            }
          }
    ...
    if (c_val && yyjson_is_arr(c_val) && yyjson_arr_size(c_val) >= 3) {
      ...
      if (caption_val && yyjson_is_arr(caption_val) &&
          yyjson_arr_size(caption_val) >= 2) {
        yyjson_val *short_val = yyjson_arr_get(caption_val, 0);
        yyjson_val *cap_blocks = yyjson_arr_get(caption_val, 1);
        if (cap_blocks && yyjson_is_arr(cap_blocks) &&
            yyjson_arr_size(cap_blocks) > 0) {
          ...
          result.push_back(CreateDocBlock(DuckBlockTypes::TYPE_CAPTION, "",
                                          cap_attrs, order++, "text",
                                          Value(effective_level + 1)));
          size_t idx, max;
          yyjson_val *cap_block;
          yyjson_arr_foreach(cap_blocks, idx, max, cap_block) {
            ProcessPandocBlockVal(cap_block, order, result, depth + 1,
                                  effective_level + 1);
          }
        }
      }
    ...
    if (c_val && yyjson_is_arr(c_val) && yyjson_arr_size(c_val) >= 2) {
      ...
      // rather than guessing from the names lining up.
      {
        ...
        static const std::set<string> SECTIONING_ROLES = {
            "section", "article", "aside", "nav", "header", "footer", "main"};
        if (!role.empty() &&
            SECTIONING_ROLES.find(role) != SECTIONING_ROLES.end()) {
  ...
  if (lineblock_lines) {
    ...
    bool any_line_rich = false;
    {
      ...
      yyjson_arr_foreach(lineblock_lines, pidx, pmax, pline) {
        ...
        vector<Value> probe;
        PandocInlineConvert::ConvertPandocInlinesValToDbInlines(
            pline, effective_level + 1, probe_order, probe, depth);
    ...
    yyjson_arr_foreach(lineblock_lines, idx, max, line) {
      if (idx > 0) {
        ...
        br.push_back(
            make_pair("element_type", Value(DuckBlockTypes::INLINE_LINEBREAK)));
        br.push_back(make_pair("content", Value("")));
        br.push_back(make_pair("level", Value(effective_level + 1)));
        br.push_back(
            make_pair("encoding", Value(DuckBlockTypes::ENCODING_TEXT)));
      ...
      PandocInlineConvert::ConvertPandocInlinesValToDbInlines(
          line, effective_level + 1, order, inline_children, depth);
    }
    if (!any_line_rich) {
    ...
    const int32_t order_before_children = order;
    PandocInlineConvert::ConvertPandocInlinesValToDbInlines(
        inlines_val_p, effective_level + 1, order, inline_children, depth);
  ...
  result.push_back(CreateDocBlock(block_type, content, attrs, block_order,
                                  encoding, block_level));
...
static void ProcessPandocMetaVal(const string &key, yyjson_val *val,
                                 int32_t &order, vector<Value> &result,
  ...
  if (strcmp(mt, "MetaString") == 0) {
    ...
    result.push_back(CreateDocValue(DuckBlockTypes::VALUE_STRING, s, attrs,
                                    order++, Value(level)));
  } else if (strcmp(mt, "MetaBool") == 0) {
    const bool b = c_val && yyjson_is_true(c_val);
    result.push_back(CreateDocValue(DuckBlockTypes::VALUE_BOOL,
                                    b ? "true" : "false", attrs, order++,
                                    Value(level)));
  } else if (strcmp(mt, "MetaInlines") == 0) {
    result.push_back(CreateDocValue(DuckBlockTypes::VALUE_INLINES, "", attrs,
                                    order++, Value(level)));
    if (c_val) {
      PandocInlineConvert::ConvertPandocInlinesValToDbInlines(
          c_val, level + 1, order, result, depth);
    }
  } else if (strcmp(mt, "MetaBlocks") == 0) {
    result.push_back(CreateDocValue(DuckBlockTypes::VALUE_BLOCKS, "", attrs,
                                    order++, Value(level)));
    if (c_val && yyjson_is_arr(c_val)) {
      size_t i, n;
      yyjson_val *b;
    ...
    result.push_back(CreateDocValue(DuckBlockTypes::VALUE_LIST, "", attrs,
                                    order++, Value(level)));
    if (c_val && yyjson_is_arr(c_val)) {
      size_t i, n;
      yyjson_val *e;
    ...
    result.push_back(CreateDocValue(DuckBlockTypes::VALUE_MAP, "", attrs,
                                    order++, Value(level)));
    if (c_val && yyjson_is_obj(c_val)) {
      size_t i, n;
      yyjson_val *k, *v;
      yyjson_obj_foreach(c_val, i, n, k, v) {
        ProcessPandocMetaVal(string(yyjson_get_str(k), yyjson_get_len(k)), v,
                             order, result, level + 1, depth + 1);
      }
    }
  } else {
    ...
    result.push_back(CreateDocValue(DuckBlockTypes::TYPE_GENERIC,
                                    ValToJsonString(val), attrs, order++,
                                    Value(level)));
  }
}

void PandocBlockConvert::ConvertPandocAstToBlocks(const string &json,
                                                  vector<Value> &blocks) {
  if (json.empty()) {
    return;
  }
...
void PandocBlockConvert::PandocAstToBlocksFun(DataChunk &args,
  ...
  for (idx_t i = 0; i < count; i++) {
    ...
    if (json_val.IsNull()) {
      result.SetValue(
          i, Value::LIST(DuckBlockTypes::DuckBlockType(), vector<Value>()));
      continue;
    }
    ...
    result.SetValue(
        i, Value::LIST(DuckBlockTypes::DuckBlockType(), std::move(blocks)));
  }
}
...
static yyjson_mut_val *CreatePandocAttrVal(yyjson_mut_doc *doc,
                                           const Value &element,
                                           const string &fallback_class) {
...
static yyjson_mut_val *ConvertListToPandocVal(yyjson_mut_doc *doc,
                                              const vector<Value> &blocks_list,
                                              idx_t &start_idx,
                                              int32_t list_level, idx_t depth);
static void ConvertContainerChildrenToPandocVal(
    yyjson_mut_doc *doc, const vector<Value> &blocks_list, idx_t &start_idx,
    int32_t parent_level, idx_t depth, yyjson_mut_val *target_arr,
    yyjson_mut_val *switch_arr, const char *switch_type);
static yyjson_mut_val *ConvertDivToPandocVal(yyjson_mut_doc *doc,
                                             const vector<Value> &blocks_list,
                                             idx_t &start_idx,
...
static yyjson_mut_val *ConvertUnhandledChildToPandocVal(
    yyjson_mut_doc *doc, const vector<Value> &blocks_list, idx_t &idx,
    idx_t depth, const Value &child, const string &child_type,
    const string &content, const vector<Value> &inline_children,
    int32_t child_level) {
  ...
  if (child_type == DuckBlockTypes::TYPE_HR) {
    ...
    idx_t skip = idx;
    ConvertContainerChildrenToPandocVal(doc, blocks_list, skip, child_level,
                                        depth + 1, yyjson_mut_arr(doc), nullptr,
                                        nullptr);
    idx = skip;
    return hr_obj;
  }
  ...
  ConvertContainerChildrenToPandocVal(doc, blocks_list, idx, child_level,
                                      depth + 1, fb_blocks, nullptr, nullptr);
...
static yyjson_mut_val *ConvertListToPandocVal(yyjson_mut_doc *doc,
  ...
  auto list_type =
      GetElementAttribute(list_block, DuckBlockTypes::ATTR_LIST_TYPE);
  auto ordered_attr =
      GetElementAttribute(list_block, DuckBlockTypes::ATTR_ORDERED_LEGACY);
  bool is_ordered = (list_type == DuckBlockTypes::LIST_TYPE_ORDERED) ||
                    (ordered_attr == "true");
  const bool is_definition =
      (list_type == DuckBlockTypes::LIST_TYPE_DEFINITION);
  const char *pandoc_type = is_definition
                                ? "DefinitionList"
                                : (is_ordered ? "OrderedList" : "BulletList");
  ...
  while (j < blocks_list.size()) {
    ...
    auto child_type =
        GetElementStringField(child, DuckBlockTypes::ELEMENT_TYPE_IDX);
    int32_t child_level = GetElementLevel(child);

    if (child_kind == DuckBlockTypes::KIND_BLOCK) {
      if (child_type == DuckBlockTypes::TYPE_LIST_ITEM &&
          child_level == list_level + 1) {
        if (in_item) {
          items.push_back(current_item);
        }
        current_item = ListItem();
        current_item.content =
            GetElementStringField(child, DuckBlockTypes::CONTENT_IDX);
        current_item.role =
            GetElementAttribute(child, DuckBlockTypes::ATTR_ROLE);
        in_item = true;
        j++;
        ...
        if (in_item) {
          current_item.nested_list_val = ConvertListToPandocVal(
              doc, blocks_list, j, child_level, depth + 1);
        } else {
          j++;
        }
      } else if ((child_type == DuckBlockTypes::TYPE_PARAGRAPH ||
                  child_type == DuckBlockTypes::TYPE_PLAIN) &&
        ...
        if (current_item.content.empty() &&
            current_item.extra_paragraphs.empty()) {
          current_item.tight = (child_type == DuckBlockTypes::TYPE_PLAIN);
          current_item.content = para_text;
        } else {
          current_item.extra_paragraphs.push_back(
              {para_text, child_type == DuckBlockTypes::TYPE_PLAIN});
        }
        j++;
        ...
        for (idx_t k = j + 1; k < blocks_list.size(); k++) {
          ...
          if (GetElementStringField(inl, DuckBlockTypes::KIND_IDX) !=
                  DuckBlockTypes::KIND_INLINE ||
        ...
        stray_item.extra_blocks.push_back(ConvertUnhandledChildToPandocVal(
            doc, blocks_list, j, depth, child, child_type, stray_content,
            stray_inlines, child_level));
        ...
        current_item.extra_blocks.push_back(ConvertUnhandledChildToPandocVal(
            doc, blocks_list, j, depth, child, child_type, child_content,
            child_inlines, child_level));
      } else {
        j++;
      }
  ...
  if (is_definition) {
    ...
    for (idx_t k = 0; k < items.size();) {
      ...
      yyjson_mut_obj_add_strncpy(doc, term_str, "c", items[k].content.data(),
                                 items[k].content.size());
      ...
      idx_t d = k + 1;
      for (; d < items.size() &&
             items[d].role == DuckBlockTypes::LIST_TYPE_DEFINITION;
           d++) {
        ...
        if (!items[d].inlines.empty()) {
          ...
          if (!items[d].content.empty()) {
            ...
            yyjson_mut_obj_add_strncpy(doc, s, "c", items[d].content.data(),
                                       items[d].content.size());
  ...
  if (is_ordered) {
    ...
    yyjson_mut_arr_add_int(
        doc, order_spec,
        ParseInt32OrDefault(GetElementAttribute(list_block, "start"), 1));
    ...
    yyjson_mut_obj_add_strcpy(doc, style_obj, "t",
                              number_style.empty() ? "Decimal"
                                                   : number_style.c_str());
    yyjson_mut_arr_add_val(order_spec, style_obj);
    yyjson_mut_val *delim_obj = yyjson_mut_obj(doc);
    yyjson_mut_obj_add_strcpy(doc, delim_obj, "t",
                              number_delim.empty() ? "Period"
                                                   : number_delim.c_str());
  ...
  for (auto &item : items) {
    ...
    if (item.content.empty() && item.inlines.empty() &&
        !item.extra_blocks.empty()) {
    ...
    if (!item.inlines.empty()) {
      ...
      yyjson_mut_val *inl_arr =
          PandocInlineConvert::ConvertDbInlinesToPandocVal(
              doc, item.inlines, 0, list_level + 2, inl_end, 1);
      ...
      yyjson_mut_obj_add_strncpy(doc, str_obj, "c", item.content.data(),
                                 item.content.size());
  ...
  if (cells_arr && yyjson_is_arr(cells_arr)) {
    ...
    yyjson_arr_foreach(cells_arr, ci, cmax, cv) {
      yyjson_mut_arr_add_val(
          cells,
          TableCellVal(doc, yyjson_is_str(cv) ? yyjson_get_str(cv) : ""));
    }
  }
...
static yyjson_mut_val *NativeTableToPandocVal(yyjson_mut_doc *doc,
                                              const string &content) {
  yyjson_mut_val *c = yyjson_mut_arr(doc);
  yyjson_doc *src = content.empty()
                        ? nullptr
  ...
  size_t ncols =
      (headers && yyjson_is_arr(headers)) ? yyjson_arr_size(headers) : 0;
  if (rows && yyjson_is_arr(rows)) {
    size_t ri, rmax;
    yyjson_val *rv;
...
ConvertFigureToPandocVal(yyjson_mut_doc *doc, const vector<Value> &blocks_list,
...
static void ConvertContainerChildrenToPandocVal(
  ...
  // position.
  {
    auto own_content = GetElementStringField(blocks_list[start_idx],
                                             DuckBlockTypes::CONTENT_IDX);
    if (!own_content.empty()) {
      ...
      yyjson_mut_val *inl_arr =
          PandocInlineConvert::ConvertDbInlinesToPandocVal(
              doc, blocks_list, start_idx + 1, parent_level + 1, inl_end,
              depth + 1);
      if (!inl_arr || yyjson_mut_arr_size(inl_arr) == 0) {
        ...
        yyjson_mut_val *str_obj = yyjson_mut_obj(doc);
        yyjson_mut_obj_add_str(doc, str_obj, "t", "Str");
        yyjson_mut_obj_add_strncpy(doc, str_obj, "c", own_content.data(),
                                   own_content.size());
        yyjson_mut_arr_add_val(inl_arr, str_obj);
      }
      yyjson_mut_obj_add_val(doc, plain_obj, "c", inl_arr);
  ...
  while (j < blocks_list.size()) {
    ...
    // unaffected.
    if (switch_type && switch_arr && child_kind == DuckBlockTypes::KIND_BLOCK &&
        child_type == switch_type && child_level == parent_level + 1) {
    ...
    if (child_level <= parent_level &&
        child_kind == DuckBlockTypes::KIND_BLOCK) {
      break;
    }
    if (child_kind == DuckBlockTypes::KIND_INLINE ||
        child_type == DuckBlockTypes::TYPE_LIST_ITEM) {
      j++;
      continue;
    }
    ...
    if (child_kind == DuckBlockTypes::KIND_BLOCK) {
      ...
      if (child_type == DuckBlockTypes::TYPE_PARAGRAPH ||
          child_type == DuckBlockTypes::TYPE_PLAIN) {
        ...
        yyjson_mut_obj_add_str(
            doc, para_obj, "t",
            child_type == DuckBlockTypes::TYPE_PLAIN ? "Plain" : "Para");
        if (!inline_children.empty()) {
          ...
          yyjson_mut_val *str_obj = yyjson_mut_obj(doc);
          yyjson_mut_obj_add_str(doc, str_obj, "t", "Str");
          yyjson_mut_obj_add_strncpy(doc, str_obj, "c", content.data(),
                                     content.size());
          yyjson_mut_arr_add_val(inl_arr, str_obj);
          yyjson_mut_obj_add_val(doc, para_obj, "c", inl_arr);
        }
        ...
        auto level_str =
            GetElementAttribute(child, DuckBlockTypes::ATTR_HEADING_LEVEL);
        ...
        yyjson_mut_arr_add_val(cc_arr,
                               CreatePandocAttrVal(doc, child, language));
        ...
        auto child_encoding =
            GetElementStringField(child, DuckBlockTypes::ENCODING_IDX);
        yyjson_mut_val *bq_obj = yyjson_mut_obj(doc);
        yyjson_mut_obj_add_str(doc, bq_obj, "t", "BlockQuote");
        if (child_encoding == DuckBlockTypes::ENCODING_JSON &&
            !content.empty()) {
          yyjson_doc *sub_doc = yyjson_read(content.c_str(), content.size(), 0);
          if (sub_doc) {
            yyjson_mut_val *imported =
                yyjson_val_mut_copy(doc, yyjson_doc_get_root(sub_doc));
        ...
        yyjson_mut_val *list_obj =
            ConvertListToPandocVal(doc, blocks_list, j, child_level, depth + 1);
        yyjson_mut_arr_add_val(child_blocks_arr, list_obj);
      } else if (child_type == DuckBlockTypes::TYPE_DIV) {
        yyjson_mut_val *nested_div_obj =
            ConvertDivToPandocVal(doc, blocks_list, j, child_level, depth + 1);
        yyjson_mut_arr_add_val(child_blocks_arr, nested_div_obj);
      } else if ((child_type == DuckBlockTypes::TYPE_TABLE ||
                  child_type == DuckBlockTypes::TYPE_DEFLIST) &&
                 GetElementStringField(child, DuckBlockTypes::ENCODING_IDX) ==
                     DuckBlockTypes::ENCODING_JSON &&
                 !content.empty()) {
        ...
        if (child_type == DuckBlockTypes::TYPE_TABLE) {
          ...
          yyjson_mut_obj_add_val(doc, obj, "c",
                                 NativeTableToPandocVal(doc, content));
        } else {
        ...
        yyjson_mut_arr_add_val(
            child_blocks_arr, ConvertUnhandledChildToPandocVal(
                                  doc, blocks_list, j, depth, child, child_type,
...
static yyjson_mut_val *ConvertDivToPandocVal(yyjson_mut_doc *doc,
  ...
  ConvertContainerChildrenToPandocVal(doc, blocks_list, start_idx, div_level,
                                      depth, child_blocks_arr, nullptr,
                                      nullptr);
  return div_obj;
}
...
static bool HasBlockChildren(const vector<Value> &blocks_list, idx_t idx) {
  ...
  for (idx_t j = idx + 1; j < blocks_list.size(); j++) {
    ...
    if (GetElementStringField(child, DuckBlockTypes::KIND_IDX) !=
        DuckBlockTypes::KIND_BLOCK) {
      continue;
    }
...
// body.
static yyjson_mut_val *
ConvertBlockquoteToPandocVal(yyjson_mut_doc *doc,
                             const vector<Value> &blocks_list, idx_t &start_idx,
                             int32_t quote_level, idx_t depth) {
  CheckPandocDepth(depth);
  yyjson_mut_val *bq_obj = yyjson_mut_obj(doc);
  yyjson_mut_obj_add_str(doc, bq_obj, "t", "BlockQuote");
  yyjson_mut_val *child_blocks_arr = yyjson_mut_arr(doc);
  yyjson_mut_obj_add_val(doc, bq_obj, "c", child_blocks_arr);
  ConvertContainerChildrenToPandocVal(doc, blocks_list, start_idx, quote_level,
                                      depth, child_blocks_arr, nullptr,
                                      nullptr);
  return bq_obj;
}
...
ConvertFigureToPandocVal(yyjson_mut_doc *doc, const vector<Value> &blocks_list,
  ...
  for (idx_t k = start_idx + 1; k < blocks_list.size(); k++) {
    ...
    if (GetElementStringField(c, DuckBlockTypes::KIND_IDX) ==
            DuckBlockTypes::KIND_BLOCK &&
        GetElementLevel(c) <= fig_level) {
      break;
    }
    if (GetElementStringField(c, DuckBlockTypes::ELEMENT_TYPE_IDX) ==
  ...
  ConvertContainerChildrenToPandocVal(doc, blocks_list, start_idx, fig_level,
                                      depth, content_arr, caption_arr,
                                      DuckBlockTypes::TYPE_CAPTION);
  ...
  if (short_text.empty()) {
    ...
    yyjson_mut_val *str_obj = yyjson_mut_obj(doc);
    yyjson_mut_obj_add_str(doc, str_obj, "t", "Str");
    yyjson_mut_obj_add_strncpy(doc, str_obj, "c", short_text.data(),
                               short_text.size());
...
static string BuildBlocksJson(const vector<Value> &blocks_in) {
  ...
  for (idx_t block_idx = 0; block_idx < blocks_list.size();) {
    ...
    auto element_type =
        GetElementStringField(block, DuckBlockTypes::ELEMENT_TYPE_IDX);
    auto content = GetElementStringField(block, DuckBlockTypes::CONTENT_IDX);

    if (element_type == DuckBlockTypes::TYPE_LIST_ITEM ||
        element_type == DuckBlockTypes::TYPE_METADATA) {
      block_idx++;
      continue;
    }
    ...
    if (element_type == DuckBlockTypes::TYPE_HEADING) {
      auto level_str =
          GetElementAttribute(block, DuckBlockTypes::ATTR_HEADING_LEVEL);
      int level = ParseInt32OrDefault(level_str, 1);
      yyjson_mut_val *header_obj = yyjson_mut_obj(doc);
      yyjson_mut_obj_add_str(doc, header_obj, "t", "Header");
      yyjson_mut_val *hc_arr = yyjson_mut_arr(doc);
      ...
      if (!inline_children.empty()) {
        ...
        yyjson_mut_val *inl_arr =
            PandocInlineConvert::ConvertDbInlinesToPandocVal(
                doc, inline_children, 0, 2, inl_end, 1);
        yyjson_mut_arr_add_val(hc_arr, inl_arr);
      } else {
        yyjson_mut_val *inl_arr = yyjson_mut_arr(doc);
        yyjson_mut_val *str_obj = yyjson_mut_obj(doc);
      ...
      block_idx++;
               ...
               GetElementStringField(block, DuckBlockTypes::ENCODING_IDX) ==
                   DuckBlockTypes::ENCODING_JSON &&
               !content.empty()) {
      yyjson_mut_val *bq_obj = yyjson_mut_obj(doc);
      yyjson_mut_obj_add_str(doc, bq_obj, "t", "BlockQuote");
      ...
      yyjson_mut_arr_add_val(
          blocks_arr, ConvertBlockquoteToPandocVal(doc, blocks_list, block_idx,
                                                   quote_level, 1));
    } else if (element_type == DuckBlockTypes::TYPE_BLOCKQUOTE) {
      yyjson_mut_val *bq_obj = yyjson_mut_obj(doc);
      yyjson_mut_obj_add_str(doc, bq_obj, "t", "BlockQuote");
      ...
      bool json_ordered =
          (GetElementAttribute(block, DuckBlockTypes::ATTR_LIST_TYPE) ==
           DuckBlockTypes::LIST_TYPE_ORDERED) ||
          (GetElementAttribute(block, DuckBlockTypes::ATTR_ORDERED_LEGACY) ==
           "true");
      yyjson_mut_val *list_obj = yyjson_mut_obj(doc);
      yyjson_mut_obj_add_str(doc, list_obj, "t",
                             json_ordered ? "OrderedList" : "BulletList");
      yyjson_mut_val *items_arr = yyjson_mut_arr(doc);
      yyjson_doc *sub_doc = yyjson_read(content.c_str(), content.size(), 0);
      if (sub_doc) {
      ...
      yyjson_mut_val *list_obj =
          ConvertListToPandocVal(doc, blocks_list, block_idx, list_level, 1);
      yyjson_mut_arr_add_val(blocks_arr, list_obj);
    } else if (element_type == DuckBlockTypes::TYPE_DIV) {
      int32_t div_level = GetElementLevel(block);
      yyjson_mut_val *div_obj =
          ConvertDivToPandocVal(doc, blocks_list, block_idx, div_level, 1);
      ...
      yyjson_mut_arr_add_val(
          sec_c,
          CreatePandocAttrVal(doc, block, role.empty() ? "section" : role));
      ...
      ConvertContainerChildrenToPandocVal(doc, blocks_list, block_idx,
                                          sec_level, 1, sec_children, nullptr,
                                          nullptr);
      ...
      yyjson_mut_val *fig_obj =
          ConvertFigureToPandocVal(doc, blocks_list, block_idx, fig_level, 1);
      yyjson_mut_arr_add_val(blocks_arr, fig_obj);
    } else if (element_type == DuckBlockTypes::TYPE_TABLE &&
               !GetElementAttribute(block, DuckBlockTypes::ATTR_PANDOC_AST)
                    .empty()) {
      ...
      yyjson_mut_val *tbl_obj = yyjson_mut_obj(doc);
      yyjson_mut_obj_add_str(doc, tbl_obj, "t", "Table");
      yyjson_mut_obj_add_val(doc, tbl_obj, "c",
                             NativeTableToPandocVal(doc, content));
      yyjson_mut_arr_add_val(blocks_arr, tbl_obj);
      block_idx++;
      ...
      if (!inline_children.empty()) {
        idx_t inl_end = 0;
        yyjson_mut_obj_add_val(doc, plain_obj, "c",
                               PandocInlineConvert::ConvertDbInlinesToPandocVal(
                                   doc, inline_children, 0, 2, inl_end, 1));
      } else {
        yyjson_mut_val *inl_arr = yyjson_mut_arr(doc);
      ...
      if (!inline_children.empty()) {
        ...
        auto flush_line = [&]() {
          idx_t inl_end = 0;
          yyjson_mut_arr_add_val(
              lines_arr, one_line.empty()
                             ? yyjson_mut_arr(doc)
                             : PandocInlineConvert::ConvertDbInlinesToPandocVal(
                                   doc, one_line, 0, GetElementLevel(block) + 1,
                                   inl_end, 1));
          one_line.clear();
        };
      ...
      while (line_start <= content.size()) {
        ...
        string line = content.substr(
            line_start, nl == string::npos ? string::npos : nl - line_start);
        yyjson_mut_val *line_arr = yyjson_mut_arr(doc);
        if (!line.empty()) {
          yyjson_mut_val *str_obj = yyjson_mut_obj(doc);
          yyjson_mut_obj_add_str(doc, str_obj, "t", "Str");
          yyjson_mut_obj_add_strncpy(doc, str_obj, "c", line.data(),
                                     line.size());
      ...
      if (sub_doc) {
        yyjson_mut_arr_add_val(
            blocks_arr, yyjson_val_mut_copy(doc, yyjson_doc_get_root(sub_doc)));
        yyjson_doc_free(sub_doc);
      } else {
        ...
        yyjson_mut_arr_add_val(
            ...
            CreatePandocAttrVal(
                doc, block,
                GetElementAttribute(block, DuckBlockTypes::ATTR_SOURCE_TYPE)));
...
static LogicalType GetPandocAstType() {
  ...
  struct_children.push_back(
      make_pair("pandoc-api-version", LogicalType::LIST(LogicalType::INTEGER)));
...
static yyjson_mut_val *BuildMetaValueJson(yyjson_mut_doc *doc,
                                          const vector<Value> &blocks_list,
                                          idx_t &i, int32_t my_level,
                                          idx_t depth) {
  CheckPandocDepth(depth);
  ...
  if (vtype == DuckBlockTypes::TYPE_GENERIC) {
    ...
    if (parsed) {
      yyjson_mut_val *copied =
          yyjson_val_mut_copy(doc, yyjson_doc_get_root(parsed));
  ...
  if (vtype == DuckBlockTypes::VALUE_INLINES) {
    ...
    idx_t end_idx = 0;
    yyjson_mut_val *arr =
        inls.empty() ? yyjson_mut_arr(doc)
                     : PandocInlineConvert::ConvertDbInlinesToPandocVal(
                           doc, inls, 0, my_level + 1, end_idx, depth + 1);
  ...
  yyjson_mut_obj_add_str(doc, obj, "t",
                         is_map ? "MetaMap"
                                : (is_blocks ? "MetaBlocks" : "MetaList"));
  yyjson_mut_val *container =
      is_map ? yyjson_mut_obj(doc) : yyjson_mut_arr(doc);
  ...
  while (i < blocks_list.size()) {
    ...
    yyjson_mut_val *v =
        BuildMetaValueJson(doc, blocks_list, i, child_level, depth + 1);
    if (!v) {
      continue;
    }
    if (is_map) {
      yyjson_mut_obj_add(container,
                         yyjson_mut_strncpy(doc, key.data(), key.size()), v);
    } else {
...
static string BuildMetaJson(const vector<Value> &blocks_list) {
  ...
  while (i < blocks_list.size()) {
    ...
    if (GetElementStringField(el, DuckBlockTypes::KIND_IDX) !=
            DuckBlockTypes::KIND_VALUE ||
    ...
    if (v && !key.empty()) {
      yyjson_mut_obj_add(root, yyjson_mut_strncpy(doc, key.data(), key.size()),
                         v);
    }
  }
...
static void DuckBlocksToPandocAstFun(DataChunk &args, ExpressionState &state,
                                     Vector &result) {
  ...
  for (idx_t i = 0; i < count; i++) {
    ...
    vector<Value> api_version_vals = {
        Value::INTEGER(pandoc_ast::API_VERSION_MAJOR),
...
void PandocBlockConvert::DuckBlocksToPandocBlocksFun(DataChunk &args,
                                                     ExpressionState &state,
                                                     Vector &result) {
  auto &blocks_vec = args.data[0];
  auto count = args.size();
...
static yyjson_mut_val *MapToMetaObj(yyjson_mut_doc *doc,
                                    const Value &meta_map) {
...
void PandocBlockConvert::ReadPandocAstFun(DataChunk &args,
                                          ExpressionState &state,
                                          Vector &result) {
...
static unique_ptr<FunctionData> PandocAstBind(ClientContext &context,
...
static void PandocAstFunction(ClientContext &context,
...
static void PandocAstJsonFun(DataChunk &args, ExpressionState &state,
                             Vector &result) {
  ...
  for (idx_t i = 0; i < args.size(); i++) {
    ...
    if (blocks_val.IsNull()) {
      result.SetValue(i, Value("{\"pandoc-api-version\":" + api_version +
                               ",\"meta\":{},\"blocks\":[]}"));
      continue;
    }
    auto &blocks_list = ListValue::GetChildren(blocks_val);
    result.SetValue(i,
                    Value("{\"pandoc-api-version\":" + api_version +
                          ",\"meta\":" + BuildMetaJson(blocks_list) +
                          ",\"blocks\":" + BuildBlocksJson(blocks_list) + "}"));
  }
}

static void WritePandocAstFun(DataChunk &args, ExpressionState &state,
                              Vector &result) {
  auto &path_vec = args.data[0];
  ...
  for (idx_t i = 0; i < count; i++) {
    ...
    if (blocks_val.IsNull()) {
      file << "{\"pandoc-api-version\":" << api_version
           << ",\"meta\":{},\"blocks\":[]}";
      file.close();
    ...
    file << "{\"pandoc-api-version\":" << api_version
         << ",\"meta\":" << meta_json << ",\"blocks\":" << blocks_json << "}";
    file.close();
    result.SetValue(i, Value(true));
  }
}
...
void PandocBlockConvert::Register(ExtensionLoader &loader) {
  ...
  auto ast_type = LogicalType::STRUCT(
      {{"pandoc-api-version", LogicalType::LIST(LogicalType::INTEGER)},
  ...
  auto to_ast = ScalarFunction("panduck_blocks_to_pandoc_ast", {blocks_type},
                               ast_type, DuckBlocksToPandocAstFun);
  ...
  to_ast.SetFallible();
  {
    ...
    desc.description =
        "Convert a list of duck_blocks into a Pandoc AST document struct.";
  ...
  auto to_blocks = ScalarFunction(
      "panduck_blocks_to_pandoc_blocks", {blocks_type}, LogicalType::VARCHAR,
  ...
  // Primary name, and the deprecated v0.4.1 spelling kept working alongside it.
  {
    CreateScalarFunctionInfo info(ScalarFunction(
        "panduck_blocks_to_pandoc_json", {DuckBlockTypes::DuckBlockListType()},
        LogicalType::VARCHAR, PandocAstJsonFun));
    info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
    FunctionDescription desc;
    desc.parameter_names = {"blocks"};
    desc.description =
        "Convert a list of duck_blocks into a Pandoc AST JSON string.";
  ...
  {
    CreateScalarFunctionInfo info(ScalarFunction(
        "panduck_pandoc_ast_json", {DuckBlockTypes::DuckBlockListType()},
        LogicalType::VARCHAR, PandocAstJsonFun));
    info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
    FunctionDescription desc;
    desc.parameter_names = {"blocks"};
  ...
  auto write_ast = ScalarFunction("panduck_write_pandoc_ast",
                                  {LogicalType::VARCHAR, blocks_type},
  ...
  write_ast.SetFallible();
  {
    ...
    desc.description =
        "Write a list of duck_blocks out to disk as a Pandoc AST JSON file.";
