...
bool IsSkippedDestination(const std::string &word) {
  return word == "fonttbl" || word == "colortbl" || word == "info" ||
         word == "object" || word == "header" || word == "footer" ||
         word == "footnote" || word == "listtable" ||
         word == "listoverridetable" || word == "rsidtbl" ||
         word == "generator" || word == "themedata" ||
         word == "colorschememapping" || word == "latentstyles" ||
         word == "datastore" || word == "xmlnstbl" || word == "upr" ||
         word == "annotation";
}
...
struct CharFormat {
  ...
  bool Plain() const { return !bold && !italic && !underline && !strike; }
...
struct GroupState {
  ...
  std::string
      meta_key; //!< non-empty: text goes to metadata under this pandoc key
};

class RtfParser {
public:
  explicit RtfParser(const std::string &src) : src_(src) { ScanListTable(); }
  ...
  std::vector<RtfBlock> Parse() {
    ...
    for (auto &kv : meta_) {
      ...
      text =
          (b == std::string::npos) ? std::string() : text.substr(b, e - b + 1);
      RtfBlock block;
...
private:
      ...
      if (c == '\'') {
        if (pos_ + 1 < src_.size()) {
          ...
          if (skip_chars_ > 0) {
            ...
            auto byte =
                static_cast<uint8_t>(std::strtol(hex.c_str(), nullptr, 16));
            std::string utf8;
  ...
  void ApplyControlWord(const std::string &word, int param, bool has_param,
                        bool was_fresh) {
    auto &g = stack_.back();
    ...
    // another's.
    if (g.in_info &&
        (word == "title" || word == "author" || word == "subject")) {
      g.skip = false;
      g.meta_key = word;
      return;
    }
  ...
  void FlushParagraph() {
    ...
    if (list_depth > 0 &&
        open_ordered_.size() == static_cast<size_t>(list_depth) &&
    ...
    while (open_ordered_.size() < static_cast<size_t>(list_depth)) {
      ...
      l.list_type = want_ordered ? DuckBlockTypes::LIST_TYPE_ORDERED
  ...
  std::map<int, std::map<int, bool>>
      list_ordered_;                //!< listid -> level -> ordered
...
unique_ptr<FunctionData> RtfReaderBind(ClientContext &context,
...
void RtfReaderScan(ClientContext &, TableFunctionInput &input,
                   DataChunk &output) {
...
void RegisterRtfReaderFunction(ExtensionLoader &loader) {
  TableFunction fn("read_rtf_blocks", {LogicalType::VARCHAR}, RtfReaderScan,
                   RtfReaderBind, RtfReaderGlobalState::Init);
