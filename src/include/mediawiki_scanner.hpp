...
//! `#`, `;`,
//! `:`, `!`, `|`, and a leading space. So this scans lines rather than
//! tokenizing characters, and inline markup is a separate pass over a line's
//! text.
//!
//! WITH ONE DEPARTURE THAT THE FORMAT FORCES. A `|` at the start of a line is a
//! table cell; a `|` inside `{{...}}` is an argument separator. The two are
//! indistinguishable by prefix, so this scanner cannot be purely line-local: it
//! tracks `{{`/`}}` and `{|`/`|}` nesting as it walks. Templates NEST --
//! `{{a|{{b|x}}|y}}` is one call, measured against pandoc -- so that tracking
//! is brace BALANCING, not matching.
//!
//! Because balancing is character-level work, it lives here rather than in the
...
//! line is TEXT, and an unterminated template or table runs to end of input
//! rather than throwing.
std::vector<Line> ScanMediaWiki(const std::string &src);

//! Split a table cell line on `||` / `!!`, the inline cell separators. Exposed
//! because the reader needs it and it is pure string work with no state.
