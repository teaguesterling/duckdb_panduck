...
//! deeper.
void PushWrapped(std::vector<MwInline> &out, const char *type,
  ...
  while (i < s.size()) {
    ...
    if (s.compare(i, 2, "[[") == 0) {
      ...
      if (close != std::string::npos) {
        ...
        std::string target =
            bar == std::string::npos ? inner : inner.substr(0, bar);
        std::string label =
    ...
    if (s[i] == '[' && s.compare(i, 2, "[[") != 0) {
      ...
      if (close != std::string::npos) {
        ...
        if (target.compare(0, 4, "http") == 0 ||
          ...
          node.element_type = DuckBlockTypes::INLINE_LINK;
          node.content =
              sp == std::string::npos ? target : Trim(inner.substr(sp + 1));
          node.level = level;
          node.attributes["href"] = target;
          out.push_back(node);
          i = close + 1;
    ...
    if (s.compare(i, 5, "'''''") == 0) {
      ...
      if (close != std::string::npos) {
        ...
        bold.level = level;
        out.push_back(bold);
        PushWrapped(out, DuckBlockTypes::INLINE_ITALIC,
                    s.substr(i + 5, close - i - 5), level + 1);
        i = close + 5;
        continue;
      }
    }
    if (s.compare(i, 3, "'''") == 0) {
      ...
      if (close != std::string::npos) {
        PushText(out, pending, level);
        pending.clear();
        PushWrapped(out, DuckBlockTypes::INLINE_BOLD,
                    s.substr(i + 3, close - i - 3), level);
        i = close + 3;
        continue;
      }
    }
...
bool FileLink(const std::string &t, std::vector<MwBlock> &out) {
  ...
  static const char *kOptions[] = {"thumb",  "thumbnail", "frame", "frameless",
...
private:
  ...
  void HtmlBlock(const Line &ln) {
    ...
    if (ln.name == "syntaxhighlight" || ln.name == "source" ||
        ln.name == "pre") {
      MwBlock c;
    ...
    r.content =
        ln.text.empty()
            ? "<" + ln.name + (ln.attrs.empty() ? "" : " " + ln.attrs) + "/>"
  ...
  void ListItem(const Line &ln) {
    ...
    if (last == ';') {
      ...
      item.attributes[DuckBlockTypes::ATTR_ROLE] =
  ...
  std::string pre_;
  std::string
      open_; //!< the marker chars of currently open lists, outermost first
};
...
unique_ptr<FunctionData> MwFileBind(ClientContext &context,
...
unique_ptr<FunctionData> MwStringBind(ClientContext &context,
                                      TableFunctionBindInput &input,
                                      vector<LogicalType> &return_types,
                                      panduck::BindNames &names) {
  readers::RequireReaderEnabled(context, "mediawiki");
...
void RegisterMediaWikiReader(ExtensionLoader &loader) {
  {
    TableFunction file_fn("read_mediawiki_blocks", {LogicalType::VARCHAR},
                          MwScan, MwFileBind, MwGlobalState::Init);
  ...
  {
    TableFunction string_fn("read_mediawiki_blocks_string",
                            {LogicalType::VARCHAR}, MwScan, MwStringBind,
                            MwGlobalState::Init);
